 ...
 */
export async function denoAdd(
  options: DenoAddOptions,
): Promise<DenoCommandResult> {
  const { packages, dev = false, workingDirectory, envVars, timeout } = options;
 ...
 */
export async function denoRemove(
  options: DenoRemoveOptions,
): Promise<DenoCommandResult> {
  const { packages, workingDirectory, envVars, timeout } = options;
 ...
 */
export async function denoOutdated(
  options: {
    update?: boolean;
    workingDirectory?: string;
    envVars?: Record<string, string>;
  ...
  const { update = false, workingDirectory, envVars, timeout } = options;
 ...
 */
export async function denoInit(
  options: {
    name?: string;
    lib?: boolean;
    serve?: boolean;
  ...
  deno_install: {
    ...
    inputSchema: {
      ...
      properties: {
        script: {
          type: "string",
          description: "Script URL or path to install",
        },
  ...
  deno_uninstall: {
    ...
    inputSchema: {
      ...
      properties: {
        name: {
          type: "string",
          description: "Name of the script to uninstall",
        },
        global: {
          type: "boolean",
          description: "Uninstall from global location",
        },
        root: { type: "string", description: "Installation root directory" },
        workingDirectory: { type: "string", description: "Working directory" },
      },
