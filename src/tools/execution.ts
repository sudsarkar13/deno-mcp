 ...
 */
export async function denoServe(options: DenoServeOptions): Promise<DenoCommandResult> {
  const {
    script,
    port,
    host,
 ...
 */
export async function denoTask(options: DenoTaskOptions): Promise<DenoCommandResult> {
  const {
    task,
    workingDirectory,
    envVars,
  ...
  if (task) {
    ...
    if (!availableTasks[task]) {
      throw new Error(`Task "${task}" not found. Available tasks: ${Object.keys(availableTasks).join(', ')}`);
    }
 ...
 */
export async function listTasks(workingDirectory?: string): Promise<Record<string, string>> {
      ...
      try {
        if (!args.task) {
          ...
          return {
            content: [{
              type: 'text' as const,
              text: taskList ? `Available tasks:\n${taskList}` : 'No tasks defined in deno.json'
            }]
          };
        }
  ...
  deno_eval: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        typescript: { type: 'boolean', description: 'Treat code as TypeScript' },
        permissions: { type: 'array', items: { type: 'string' }, description: 'Permissions to grant' },
        workingDirectory: { type: 'string', description: 'Working directory' }
      },
      required: ['code']
