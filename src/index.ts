  ...
  if (!denoInstalled) {
    return {
      tools: [
        {
          name: 'deno_version',
          description: '⚠️ Deno not found! Use this tool to check installation status and get installation instructions.',
          inputSchema: {
            type: 'object',
            properties: {},
...
async function main() {
  try {
    ...
    console.error('Server capabilities: Comprehensive Deno CLI toolchain access');
    console.error('Available tool categories: execution, development, dependencies, compilation, utilities');
  } catch (error) {
