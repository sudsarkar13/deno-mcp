 ...
 */
export async function denoUpgrade(options: DenoUpgradeOptions): Promise<DenoCommandResult> {
  const {
    version,
    output,
    dryRun = false,
...
export async function checkDenoStatus(): Promise<{
  ...
  try {
    ...
    if (!installed) {
      return { installed: false, error: 'Deno is not installed or not accessible' };
    }
 ...
 */
export async function denoCompletions(options: {
